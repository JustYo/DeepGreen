...
class Experiment:
    def __init__(
        ...
        model_path = [".", results_folder, experiment_name, "final_model.keras"]
    ...
    def find_sample(
        self, dataset_name="train1", mode="worst", meanidx=None, index=None
    ):
        # grab the relevant data:
        u, F = self.access_data(dataset_name, index)
    ...
    def get_training_losses(self):
        ...
        train_loss = (
            self.init_train_hist["aec_hist"][best_model_idc]["loss"]
            + self.init_train_hist["full_hist"][best_model_idc]["loss"]
            + self.final_train_hist["loss"]
        )
        val_loss = (
            self.init_train_hist["aec_hist"][best_model_idc]["val_loss"]
...
from architecture.NormalizedMeanSquaredError import (
    NormalizedMeanSquaredError2D as NMSE2,
)
